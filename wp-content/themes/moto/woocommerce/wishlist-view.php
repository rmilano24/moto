...
>
    ...
    ?>
        <div class="wishlist-title <?php echo ( $wishlist_meta['is_default'] != 1 && $is_user_owner ) ? 'wishlist-title-with-form' : ''?>">
            <?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h2>' . esc_attr( $page_title ) . '</h2>' ); ?>
        ...
        if( count( $wishlist_items ) > 0 ) :
            foreach( $wishlist_items as $item ) :
                ...
                if( $product !== false && $product->exists() ) :
	                ...
	                ?>
                    <tr id="yith-wcwl-row-<?php echo esc_attr( $item['prod_id'] ); ?>" data-row-id="<?php echo $item['prod_id'] ?>">
	                    <?php if( $show_cb ) : ?>
