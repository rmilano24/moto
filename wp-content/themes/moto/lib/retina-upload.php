...
function ep_retina_upload_filter( $metadata ){
    ...
    $path     = ep_path_combine($upload['basedir'], $relPath);
    ...
    foreach( $sizes as $key => $size)
    {
        ...
        //Rename the file
        rename(ep_path_combine($path, $sizes[$key]['file']), ep_path_combine($path, $newName));
...
add_filter('wp_generate_attachment_metadata', 'ep_retina_upload_filter' );