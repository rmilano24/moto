...
function ep_get_post_parent_trail($post_id)
{
    $parents = array();
...
function ep_modify_breadcrumb_single($postType, $trail, $delimiter)
{
    ...
    if(isset($_GET['pnt']))
    {
        ...
        {
            $parents = ep_get_post_parent_trail($parent);
    ...
    else
    {
        ...
        $pages = ep_search_pages_by_content('[portfolio');
        $page  = null;
        ...
        if($page)
        {
            $parents = ep_get_post_parent_trail($page->ID);
...
add_filter('ep_breadcrumb_single_trail_filter', 'ep_modify_breadcrumb_single', 10, 3);

function ep_breadcrumb_single_trail_handler($postType)
{
    if('portfolio'==$postType)
        return true;
...
add_filter('ep_breadcrumb_single_trail_handler', 'ep_breadcrumb_single_trail_handler');