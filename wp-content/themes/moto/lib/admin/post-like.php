...
function getPostLikeLink( $post_id ) {
	...
	$output = '<a href="#" class="icon-heart5 jm-post-like'.$class.'" data-post_id="'.$post_id.'" title="'.$title.'"><span class="count' . ( (empty( $like_count ) || $like_count == "0")? ' no_like' : '' ).'">'.$count.'</span></a>';
	return $output;
}
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php _e('You Liked:', 'epicomedia' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . _e('You didn\'t like anything yet.', 'epicomedia' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
...
function frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . __('You Like:', 'epicomedia') . "</h3>\n";
			$like_list .= "<ul>\n";
